...
<head>
    ...
    <title>Kategori: <?= htmlspecialchars($category) ?></title>
...
<header>
    <div class="header-container">
        ...
        <div class="right-controls">
            <button id="back-btn" onclick="window.location.href='index.php'" style="background: none; border: none; color: #005baa; font-size: 24px; cursor: pointer; padding: 5px;" title="Kembali">
                <i class="ri-close-line"></i>
            </button>
            <div class="lang-buttons" style="display: none;">
                <button id="btn-id">ID</button>
                <span>|</span>
                <button id="btn-en">EN</button>
            </div>

            <button id="burger-btn" aria-label="Toggle Menu" style="display: none; margin-right: 10px;">
                <i class="ri-menu-line"></i>
            </button>
            <button id="search-toggle-btn" aria-label="Toggle Search" style="background:none; border:none; color:#005baa; font-size:24px; cursor:pointer; padding:5px; margin-left:10px; z-index: 1101; position: relative;">
...
<div class="page-title" style="margin-bottom: 2rem; overflow-x: hidden;">
    <h1>
    ...
    <?php if (empty($articles)): ?>
        <p class="text-center mt-4">Tidak ada artikel dalam kategori ini.</p>
    <?php else: ?>
                ...
                <div class="col-md-4 mb-4">
                    <div class="card h-100">
                        <?php if (!empty($article['featured_image'])): ?>
                            <img src="admin/uploads/articles/<?= htmlspecialchars(basename($article['featured_image'])) ?>" class="card-img-top" alt="<?= htmlspecialchars($article['title']) ?>">
                        <?php endif; ?>
                        <div class="card-body d-flex flex-column">
                            <small class="text-muted mb-2"><?= date('F d, Y', strtotime($article['created_at'])) ?></small>
                            <h5 class="card-title">
                                <a href="artikel.php?slug=<?= urlencode($article['slug']) ?>" class="text-decoration-none text-dark fw-bold">
                                    <?= htmlspecialchars($article['title']) ?>
                                </a>
                            </h5>
                            <p class="card-text" style="flex-grow: 1; max-height: 6em; overflow: hidden; text-overflow: ellipsis;">
                                <?= nl2br(htmlspecialchars(substr($article['content'], 0, 300))) ?>...
                            </p>
                            <div class="mt-auto">
                                <small class="text-muted">Ditulis oleh <strong><?= htmlspecialchars($article['author_name']) ?></strong></small>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-6 mb-4">
                ...
                <p><i class="bi bi-geo-alt-fill me-2"></i>Jl. Raya Dramaga<br>Kampus IPB Dramaga Bogor<br>16680 West Java, Indonesia</p>
                <p><i class="bi bi-telephone-fill me-2"></i>+62 251 8622642</p>
                <p><i class="bi bi-envelope-fill me-2"></i>ask@apps.ipb.ac.id</p>
